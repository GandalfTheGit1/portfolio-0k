...
export default function ReActRAGPage() {
  const blogPost = {
    slug: "react-rag",
    title: "ReAct RAG: Reasoning + Acting with Tools and Retrieval",
    excerpt:
      "Interleave chain‑of‑thought with actions (retrieval/tool calls). Route, fetch, and reason step‑by‑step for complex queries.",
    ...
    content: [
      { type: "section", id: "tldr", title: "TL;DR", content: "Think‑act loops: the model reasons, decides which tool to call (retriever, web, DB), observes results, and continues until ready to answer." },
      { type: "section", id: "definition", title: "Definition", content: "ReAct interleaves natural‑language reasoning traces with explicit actions. In RAG, actions typically include retrieval and verification tools." },
      { type: "section", id: "policy", title: "Action Policy (Typical)", isList: true, items: ["If missing info → retrieve","If ambiguous → ask clarification","If conflicting sources → fetch more + verify","Stop when confidence ≥ threshold"] },
      { type: "section", id: "tools", title: "Tools", isList: true, items: ["Retriever: vector + BM25","Web: search API","DB: SQL or Graph","Verifier: cross‑encoder or heuristic"] },
      { type: "section", id: "prompts", title: "Prompting Pattern", isList: true, items: ["System: you can call tools and must cite sources","Format: Thought → Action → Observation → …","Terminate with Answer + Citations"] },
      { type: "section", id: "risks", title: "Risks & Guards", isList: true, items: ["Looping → cap steps (≤8)","Tool abuse → budgets per session","Injection → sanitize tool inputs","Latency → parallelize where safe"] },
      { type: "section", id: "benchmarks", title: "Benchmarks (Illustrative)", isList: true, items: ["Complex multi‑hop QA: +10–18% vs single‑pass","Latency +600–1200ms","Cost +20–40% (tool calls)"] },
    ],
  }
